...
from api.model.sign_insurance_history_dev import SignInsuranceHistoryDevClass
    ...
    try:
        ...
        if partner_info.count() > 0:
            ...
            if kiosk_service_info.count() > 0:
                ...
                new_sign_insurance_history = SignInsuranceHistoryDevClass(
                    device_code=device,
                    name=encrypt_name,
                ...
                try:
                    ...
                    return response_message_handler(200, result_detail={"sign_insurance_id": new_sign_insurance_history.seq})
...
def update_sign_insurance_result(data, header):
    ...
    db = SessionLocal()
    current_time_stamp = currentUTCTimestamp()
    insurance_document_request_status = '' if 'insurance_document_request_status' not in data or \
                                              data['insurance_document_request_status'] == '' else \
        data['insurance_document_request_status']
    ...
    try:
        ...
        if partner_info.count() > 0:
            if insurance_document_request_status != '' or insurance_coverage_case != '':

                # 보험 가입내역 조회
                sign_insurance_info = db.query(SignInsuranceHistoryDevClass).filter_by(seq=sign_insurance_id, active=1)

                if sign_insurance_info.count() > 0:

                    try:

                        sign_insurance_info.first().insurance_document_request_status = insurance_document_request_status
