        ...
        if partner_info.count() > 0:
            ...
            if get_claim_info.count() > 0:
                if data['upload_target'] == 'S3':
                    ...
                    insurant_signature_file_path = f'{file_upload_path["URL"]}/{file_upload_path["DEV_BUCKET"]}' \
                    ...
                    if not os.path.exists(insurant_signature_file_path):
                        ...
                        try:
                            s3_client.upload_file(file, file_upload_path["DEV_BUCKET"], key_path)
                            """ 올려놨던 파일 삭제 """
                            try:
        ...
        if partner_info.count() > 0:
            ...
            if get_claim_info.count() > 0:
                if data['upload_target'] == 'S3':
                    ...
                    beneficiary_signature_file_path = f'{file_upload_path["URL"]}/{file_upload_path["DEV_BUCKET"]}' \
                                                   f'/signature/{data["claim_id"]}/beneficiary_signature.png'
                    if not os.path.exists(beneficiary_signature_file_path):
                        return response_message_handler(204, result_message='수익자 서명파일이 없습니다.')
                    else:
                        s3_client = boto3.client('s3',
                                                 aws_access_key_id=s3_config['AWS_ACCESS_KEY'],
                                                 aws_secret_access_key=s3_config['AWS_SECRET_KEY'],
                        ...
                        try:
                            s3_client.upload_file(file, s3_config['DEV_BUCKET_NAME'], key_path)
                            """ 올려놨던 파일 삭제 """
                            try:
